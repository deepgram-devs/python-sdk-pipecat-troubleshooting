"""
Deepgram SDK v5 Synchronous TTS Benchmark with Granular Timing (Run 3)

This script benchmarks Deepgram SDK v5's synchronous generate() method with
telemetry enabled, measuring 6 granular timing milestones to understand
exactly where latency occurs in the SDK's lazy evaluation architecture.
"""
...
def _write_results(
    ...
    base_dir = Path(args.output_dir) / "run-3" / "sdk_v5_sync_tts" / timestamp
    base_dir.mkdir(parents=True, exist_ok=True)

    payload = {
        "benchmark": "sdk_v5_sync_tts_run3",
    ...
    with (base_dir / "summary.txt").open("w", encoding="utf-8") as handle:
        handle.write("Deepgram SDK v5 sync benchmark (Run 3 - Granular Timing)\n")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark Deepgram TTS via SDK v5 synchronous mode with granular timing instrumentation (Run 3)."
    )
