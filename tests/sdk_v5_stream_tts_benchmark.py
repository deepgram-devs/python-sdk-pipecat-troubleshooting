...
async def _run_once(
    ...
    response: Optional[AsyncIterator[bytes]] = None
    try:
...
async def _write_results(
    ...
    base_dir = Path(args.output_dir) / "sdk_v5_stream_tts" / timestamp
    base_dir.mkdir(parents=True, exist_ok=True)

    payload = {
        "benchmark": "sdk_v5_stream_tts",
    ...
    with (base_dir / "summary.txt").open("w", encoding="utf-8") as handle:
        handle.write("Deepgram SDK v5 streaming benchmark\n")
        handle.write(
...
async def _run(args: argparse.Namespace) -> None:
    ...
    for idx in range(1, args.iterations + 1):
        try:
            result = await _run_once(client, args, request_options)
    ...
    print("\nSummary (Deepgram SDK v5 streaming)")
    print(
    ...
    await _write_results(metrics, summary, args)


def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark Deepgram TTS via SDK v5 streaming API."
    )
    parser.add_argument("--text", default="Testing Deepgram SDK v5 streaming TTS.")
